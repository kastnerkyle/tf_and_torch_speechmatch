...
def SequenceConv1dStack(list_of_inputs, list_of_input_dims, num_feature_maps,
    ...
    intermediates_dict["seqconv1d_{}_pre".format(name)] = c
    prev_layer = c
    for ii in range(n_stacks):
        ...
        for jj, ks in enumerate(kernel_sizes):
            c = Conv2d([prev_layer], [len(kernel_sizes) * num_feature_maps], num_feature_maps,
                       ...
                       bias_offset=bias_offset, strict=strict)
            cs.append(c)
            intermediates_dict["seqconv1d_{}_conv_{}_{}".format(name,ii, jj)] = c
        layer = tf.concat(cs, axis=-1)
        intermediates_dict["seqconv1d_{}_conv_{}_comb".format(name, ii)] = layer
        bn_l = BatchNorm2d(layer, batch_norm_flag, name="bn_conv{}".format(ii))
        intermediates_dict["seqconv1d_{}_conv_{}_bn".format(name, ii)] = layer
        r_l = ReLU(bn_l)
        intermediates_dict["seqconv1d_{}_conv_{}_relu".format(name, ii)] = layer
        prev_layer += r_l
        intermediates_dict["seqconv1d_{}_conv_{}_res".format(name, ii)] = prev_layer
    ...
    intermediates_dict["seqconv1d_{}_conv_post".format(name, ii)] = prev_layer
    return tf.transpose(post[:, 0], (1, 0, 2)), intermediates_dict
